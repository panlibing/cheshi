@echo off
rem Build portrelay.exe with MSVC. Run from anywhere.
rem MSVC environment is located automatically: an already-initialized cl.exe first,
rem then vswhere, then the usual install paths -- so this script works on any machine
rem (any VS edition / version / install location) and directly in CI.
